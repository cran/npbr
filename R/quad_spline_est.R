 quad_spline_est<-function(xtab, ytab, x, kn=ceiling((length(xtab))^(1/4)), 
  cv=0, all.dea=FALSE)
 {
  ...
  #       cv = 0 (only monotonicity) / cv=1 (monotonicity+concavity)  #
  #       all.dea = FALSE  # if cv=1, then it keeps all the points 
  #       defining the DEA enveloppe as knots 
  #----------------------------------------------------------------------

     stopifnot(cv%in%c(0,1),!(all.dea & cv==0),length(xtab)==length(ytab))
